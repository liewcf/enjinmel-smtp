# Tasks: EnjinMel SMTP for WordPress

**Input**: Design documents from `/specs/001-wordpress-enjinmel-smtp/`

> _Legacy note_: Task references to files such as `class-encryption.php` or `class-logging.php` pre-date the 2025-10-01 rename. These now map to the `class-enjinmel-smtp-*.php` equivalents and should be interpreted accordingly.

## Phase A: Core Plugin Structure & Settings
- [x] T001: Create the main plugin file `enjinmel-smtp.php` and the directory structure `includes/` and `tests/`.
- [x] T002: In `enjinmel-smtp.php`, implement the Settings API integration to create the settings page under "Settings" > "EnjinMel SMTP".
...
- [x] T006: In `enjinmel-smtp.php`, implement the `phpmailer_init` hook to configure PHPMailer with the saved SMTP settings.
...
- [x] T011: In `enjinmel-smtp.php`, implement the `register_activation_hook` to create the custom database table for email logging using `dbDelta()`.
...
- [ ] T015: [P] In `enjinmel-smtp.php`, implement the `enjinmel_smtp_before_send` and `enjinmel_smtp_after_send` action hooks.
- [ ] T016: [P] In `includes/class-logging.php`, implement the `enjinmel_smtp_log_entry` filter hook.
...
Task: "T015: In enjinmel-smtp.php, implement the enjinmel_smtp_before_send and enjinmel_smtp_after_send action hooks."
Task: "T016: In includes/class-logging.php, implement the enjinmel_smtp_log_entry filter hook."
