<?php
/**
 * Integration tests that exercise the wp_mail workflow.
 *
...
class Test_Email_Sending extends WP_UnitTestCase {
	...
	/**
	 * Reset plugin settings and hooks after each integration scenario.
	 *
	 * @return void
	 */
	...
	/**
	 * Ensure the REST payload mirrors wp_mail inputs and fires success hook.
	 *
	 * @return void
	 */
	public function test_rest_payload_includes_headers_and_triggers_success_action() {
		...
		add_action(
			'wp_mail_succeeded',
			function ( $data ) use ( &$action_payload ) {
				$action_payload = $data;
			},
			10,
			1
