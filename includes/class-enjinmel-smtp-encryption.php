...
class EnjinMel_SMTP_Encryption {
	...
	public static function encrypt( $data ) {
		...
		return 'v2:' . base64_encode( $iv . $raw ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode -- Encoding binary IV and ciphertext for storage, not obfuscating executable code.
	}
	...
	public static function decrypt( $data ) {
		...
		if ( strncmp( $data, 'v2:', 3 ) === 0 ) {
			$blob = base64_decode( substr( $data, 3 ), true ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode -- Decoding stored binary IV and ciphertext, not executable code.
			if ( false === $blob ) {
	...
	private static function get_or_create_stored_keys() {
		...
		if ( null === $key || null === $iv ) {
			...
			global $wpdb;
			$wpdb->query( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- One-time option autoload migration for plugin-managed option rows.
				$wpdb->prepare(
