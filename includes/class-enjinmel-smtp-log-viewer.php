...
class EnjinMel_SMTP_Log_Viewer {
		...
		<div class="wrap">
			...
			<div class="tablenav top">
				<div class="alignleft actions bulkactions">
					...
					<button type="button" id="doaction"
						class="button action"><?php echo esc_html__( 'Apply', 'enjinmel-smtp' ); ?></button>
				</div>
				<div class="alignleft actions">
					<button type="button" id="export-csv"
						class="button"><?php echo esc_html__( 'Export to CSV', 'enjinmel-smtp' ); ?></button>
					<button type="button" id="clear-all-logs" class="button button-secondary"
						style="margin-left: 5px;"><?php echo esc_html__( 'Clear All Logs', 'enjinmel-smtp' ); ?></button>
				</div>
				...
				<div class="filter-row">
					...
					<select name="status" id="log-status">
						...
						<option value="sent" <?php selected( $status, 'sent' ); ?>>
							<?php echo esc_html__( 'Sent', 'enjinmel-smtp' ); ?></option>
						<option value="failed" <?php selected( $status, 'failed' ); ?>>
							<?php echo esc_html__( 'Failed', 'enjinmel-smtp' ); ?></option>
					</select>
					...
					<button type="submit"
						class="button button-primary"><?php echo esc_html__( 'Filter', 'enjinmel-smtp' ); ?></button>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=enjinmel-smtp-logs' ) ); ?>"
						class="button"><?php echo esc_html__( 'Reset', 'enjinmel-smtp' ); ?></a>
				</div>
			</form>
		</div>
		<?php
			...
			<span class="pagination-links">
				<?php if ( $current_page > 1 ) : ?>
					...
					<a class="prev-page button"
						href="<?php echo esc_url( add_query_arg( 'paged', max( 1, $current_page - 1 ), $current_url ) ); ?>">
				...
				<span class="paging-input">
					<label for="current-page-selector"
						class="screen-reader-text"><?php echo esc_html__( 'Current Page', 'enjinmel-smtp' ); ?></label>
					<input class="current-page" id="current-page-selector" type="text" name="paged"
						value="<?php echo esc_attr( $current_page ); ?>"
						size="<?php echo esc_attr( strlen( (string) $total_pages ) ); ?>" aria-describedby="table-paging">
					<span class="tablenav-paging-text"> <?php echo esc_html__( 'of', 'enjinmel-smtp' ); ?> <span
							class="total-pages"><?php echo esc_html( number_format_i18n( $total_pages ) ); ?></span></span>
				</span>

				<?php if ( $current_page < $total_pages ) : ?>
					<a class="next-page button"
						href="<?php echo esc_url( add_query_arg( 'paged', min( $total_pages, $current_page + 1 ), $current_url ) ); ?>">
						<span class="screen-reader-text"><?php echo esc_html__( 'Next page', 'enjinmel-smtp' ); ?></span>
						<span aria-hidden="true">&rsaquo;</span>
					</a>
					<a class="last-page button"
						href="<?php echo esc_url( add_query_arg( 'paged', $total_pages, $current_url ) ); ?>">
	...
	public static function ajax_delete_logs() {
		...
		wp_send_json_success(
			array(
				'message' => __( 'Logs deleted successfully.', 'enjinmel-smtp' ),
				'deleted' => $deleted,
			)
		);
	}
