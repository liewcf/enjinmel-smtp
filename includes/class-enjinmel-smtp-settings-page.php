...
class EnjinMel_SMTP_Settings_Page {
	...
	public static function api_key_render() {
		...
		<div class="enjinmel-api-key-wrapper" style="display: flex; align-items: center; gap: 8px;">
			<input type='password' id='enjinmel_smtp_api_key' autocomplete='off' name='enjinmel_smtp_settings[api_key]'
				value='<?php echo esc_attr( $display_value ); ?>' placeholder='<?php echo esc_attr( $placeholder ); ?>'
				style="width: 300px;">
			<button type="button" id="enjinmel_smtp_toggle_api_key" class="button button-secondary" style="min-width: 60px;">
	...
	public static function campaign_name_render() {
		$options = enjinmel_smtp_get_settings();
		?>
		<input type='text' name='enjinmel_smtp_settings[campaign_name]'
			value='<?php echo isset( $options['campaign_name'] ) ? esc_attr( $options['campaign_name'] ) : ''; ?>'
			placeholder='<?php echo esc_attr__( 'Transactional Campaign', 'enjinmel-smtp' ); ?>'>
		<?php
	}
	...
	public static function template_id_render() {
		$options = enjinmel_smtp_get_settings();
		?>
		<input type='text' name='enjinmel_smtp_settings[template_id]'
			value='<?php echo isset( $options['template_id'] ) ? esc_attr( $options['template_id'] ) : ''; ?>'
			placeholder='<?php echo esc_attr__( 'e.g. TPL-123', 'enjinmel-smtp' ); ?>'>
		<?php
	}
	...
	public static function from_name_render() {
		$options = enjinmel_smtp_get_settings();
		?>
		<input type='text' name='enjinmel_smtp_settings[from_name]'
			value='<?php echo isset( $options['from_name'] ) ? esc_attr( $options['from_name'] ) : ''; ?>'
			placeholder='<?php echo esc_attr__( 'EnjinMel', 'enjinmel-smtp' ); ?>'>
		<?php
	}
	...
	public static function from_email_render() {
		$options = enjinmel_smtp_get_settings();
		?>
		<input type='email' name='enjinmel_smtp_settings[from_email]'
	...
	public static function send_test_email() {
		...
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Raw value is sanitized immediately below and compared against sanitized output to reject tampering.
		$raw_to = isset( $_POST['to'] ) ? trim( (string) wp_unslash( $_POST['to'] ) ) : '';
		$to     = sanitize_email( $raw_to );
		if ( empty( $to ) || $raw_to !== $to || ! is_email( $to ) ) {
